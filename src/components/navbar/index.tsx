      ...
      <div className='fixed top-0 left-0 right-0 z-40 '>
          ...
          <nav className='w-[90%] h-20 sm:max-w-6xl mx-auto flex items-center font-medium justify-between'>
            ...
            <div className='flex items-center'>
              <div className='ml-20 space-x-20 hidden lg:block '>
                ...
                <Link href={'/service'} className={`hover:text-green-800 ${pathname == '/service' ? 'text-green-600' : ''}`}>
                  Service
                </Link>
                <Link href={'/team'} className={`hover:text-green-800 ${pathname == '/team' ? 'text-green-600' : ''}`}>
                  Our Teams
                </Link>
              </div>
            </div>
            ...
            <button className='block sm:hidden text-gray-500 w-10 h-10 relative focus:outline-none bg-white' onClick={() => setOpen(prevState => !prevState)}>
              <span className='sr-only'>Open main menu</span>
              <div className='block w-5 absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2'>
                <span aria-hidden='true' className={`block absolute h-0.5 w-5 bg-current transform transition duration-500 ease-in-out ${!open && '-translate-y-1.5'} ${open && 'rotate-45'}`}></span>
        ...
        >
          <div className='flex flex-col items-start gap-y-4 w-[90%] sm:max-w-6xl mx-auto '>
            ...
            <Link href='/formulir' className='mt-5 py-2 px-5 w-full text-center text-white bg-green-600 hover:bg-green-800 rounded-md'>
              Request Project
            </Link>
          </div>
        </div>
